...
OpenClaw replies are generated by the local OpenClaw process, not by the AWS
server and not by the mobile app. A working deployment can still fail to answer
in the App Agent room when OpenClaw cannot run its selected model.

Check OpenClaw before starting the gateway:

```powershell
openclaw agent --message "Reply with only: ok"
```

If OpenClaw reports a missing API key or unknown model, run:
...
Do not configure `DIREXIO_GATEWAY_COMMAND=node` by itself. The App Agent prompt
must go through the generated `p2p_handler.cjs`, which calls `openclaw agent`.

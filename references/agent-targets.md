...
| OpenClaw | `native` | `$HOME/.openclaw/direxio/nodes/<agent_node_id>/mcp.json` | Run `openclaw plugins install ./platforms/openclaw`; register with `openclaw mcp set direxio <mcp.json>`; require channel probe, MCP probe, and Agent chat round trip before completion. |
